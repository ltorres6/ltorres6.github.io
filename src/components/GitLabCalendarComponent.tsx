...
const fetchCalendarData = async (username: string, lastYearISO: string, todayISO: string, page: number): Promise<any> => {
    const response = await fetch(`https://gitlab.com/api/v4/users/${username}/events?after=${lastYearISO}&before=${todayISO}&page=${page}&per_page=100`);
    if (!response.ok) {
        const data = await response.json();
        throw Error(`Fetching GitLab contribution data for "${username}" failed: ${data.message}`);
    }
...
const fetchAllData = async (username: string): Promise<ApiResponse> => {
    ...
    const contributions: Activity[] = Object.entries(contributionsData).map(([date, count]) => ({
        date,
        count,
        level: Math.min(4, Math.floor(count / 5)),
    }));

    return { contributions, total: { 'last': contributions.reduce((acc, { count }) => acc + count, 0) } };
};
...
const GitLabCalendarComponent: FunctionComponent<Props> = ({
    ...
    return (
        <div>
            <Calendar
                data={transformFn ? transformFn(data.contributions) : data.contributions}
                theme={theme}
