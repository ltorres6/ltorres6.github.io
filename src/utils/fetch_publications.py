...
async def fetch_publication_details(publication, session):
    ...
    for attempt in range(retries):
        try:
            ...
            logging.warning(f"Attempt {attempt + 1} failed for publication: {publication}. Error: {e}")
            await asyncio.sleep(1)
    logging.error(f"Failed to fetch details for publication after {retries} attempts: {publication}")
    return None
